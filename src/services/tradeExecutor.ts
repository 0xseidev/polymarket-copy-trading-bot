...
const TRADE_AGGREGATION_MIN_TOTAL_USD = ENV.TRADE_AGGREGATION_MIN_TOTAL_USD;
    ...
    while (isRunning) {
        ...
        if (TRADE_AGGREGATION_ENABLED) {
            ...
            if (trades.length === 0 && readyAggregations.length === 0) {
                if (Date.now() - lastCheck > ENV.WAITING_MESSAGE_INTERVAL_MS) {
            ...
            if (trades.length > 0) {
                ...
                // Update waiting message for smooth animation
                if (Date.now() - lastCheck > ENV.WAITING_MESSAGE_INTERVAL_MS) {
                    Logger.waiting(USER_ADDRESSES.length);
                    lastCheck = Date.now();
                }
        ...
        if (!isRunning) break;
        await new Promise((resolve) => setTimeout(resolve, ENV.EXECUTOR_LOOP_DELAY_MS));
    }
